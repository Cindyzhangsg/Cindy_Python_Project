...
issues
table
with the sum of numeric values in each column.
//
...
https: // www.google.com / s2 / favicons?domain = https: // www.tampermonkey.net /
//
...
function
parseCellsForIntegers(cells, array)
{
...
function
sumNumericColumnFor(gadget)
{
    addTableHead(gadget);
const
headerRow = gadget.querySelector('thead tr');
let
issueRows = gadget.querySelectorAll('tbody tr');
if (issueRows.length > 0)
{
let
sumArray = getIntegerArray(headerRow.querySelectorAll('th').length);
...
{
    const
gadgets = document.querySelectorAll(CONFIG.ISSUES_TABLES_SELECTOR.join());
...
detectEpicPage = () = > {
    var
isEpic = document.querySelectorAll('.type-gh-epic-label').length > 0;
return isEpic;
}
...
{
const
response = await fetch('/rest/api/latest/search?jql=' + jql);
const
...
{
const
epicKey = document.querySelector('.issue-link').getAttribute('data-issue-key');
...
var
issueDom = document.querySelector('#ghx-issues-in-epic-table tr[data-issuekey="' + story.key + '"]');
var
effortTD = document.createElement('td');
effortTD.classList.add('nav', 'effort');
effortTD.innerText = story.effort;
var
tdActions = issueDom.querySelector('.issue_actions');
...
load = () = > {
const
isJiraLocation = (new RegExp(CONFIG.URL_IDENTIFIER_FOR_JIRA)).test(location.href);
if (isJiraLocation) {
